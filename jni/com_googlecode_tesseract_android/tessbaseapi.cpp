 ...
 */
bool progressJavaCallback(ETEXT_DESC* ths, int left, int right,
		int top, int bottom) {
  native_data_t *nat = (native_data_t*)ths->cancel_this;
  int16_t progress = ths->progress;
...
jlong Java_com_googlecode_tesseract_android_TessPdfRenderer_nativeCreate(JNIEnv *env,
  ...
  tesseract::TessPDFRenderer* result = new tesseract::TessPDFRenderer(c_output_path, nat->api.GetDatapath(), false);
